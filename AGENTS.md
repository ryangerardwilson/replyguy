...
- `replyguy timer install` installs an hourly user timer that runs the direct inhale worker in the background. `replyguy timer disable` disables it and `replyguy timer status` shows timer status plus the current queue summary.
...
- `replyguy config` opens the user-editable JSON config in `$VISUAL`, then `$EDITOR`, then `vim`.
